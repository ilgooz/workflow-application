...
// ParseYAMLFromPath parses a workflow yml file from path to Definition.
func ParseYAMLFromPath(path string) (WorkflowDefinition, error) {
	f, err := os.Open(path)
	if err != nil {
		return WorkflowDefinition{}, err
	}
	return newYAMLParser(f).parse()
}
