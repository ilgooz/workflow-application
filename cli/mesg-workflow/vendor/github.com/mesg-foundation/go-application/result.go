...
func (e *ResultEmitter) Execute(serviceID, taskKey string, options ...ListenerOption) (*Listener, error) {
	e.taskServiceID = serviceID
	e.taskKey = taskKey
	listener := newListener(e.app, e.gracefulWait, options...)
...
func (e *ResultEmitter) execute(listener *Listener, result *Result) {
	...
	executionID, err := e.app.execute(e.taskServiceID, e.taskKey, data, executionTags)
	if err != nil {
		e.app.log.Println(executionError{e.taskKey, err})
	}
	listener.sendExecution(Execution{
